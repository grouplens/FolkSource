...
if [[ -n $COPY_HIDDEN ]]; then
  alias do_find='/usr/bin/find "$SRC_DIR" -name "*.sh" -prune -o -name node_modules -prune -o'
else
  alias do_find='/usr/bin/find -L "$SRC_DIR" -name ".*" -prune -o -name "*.sh" -prune -o -name node_modules -prune -o'
fi
