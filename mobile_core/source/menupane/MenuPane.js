...
enyo.kind({
  ...
  selectView: function(name) {
    ...
    this.bubble("onViewChanged", undefined, name);
    //this.doViewChanged({prop: name});
  },
    ...
    if (Math.abs(value) == 100) {
      ...
      if (this.$.menu.getShowing()) {
        if (inSender.getValue() == this.position.menu.max) {
          //this.bubble("onMenuOpened");
          this.doMenuOpened();
        } else {
          //this.bubble("onMenuClosed");
          this.doMenuClosed();
        }
      } else {
        if (inSender.getValue() == this.position.secondarymenu.min) {
          //this.bubble("onSecondaryMenuOpened");
          this.doSecondaryMenuOpened();
        } else {
          //this.bubble("onSecondaryMenuClosed");
